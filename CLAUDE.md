...
**This pair is the platform's reference-data template.** It exists to show how little ceremony a domain-specific reference-data need takes to stand up on the substrate, and to give anyone building their own bounded context's reference-data seeding a working pattern to point at — not a dependency anyone is obligated to take. See the staged seeding-framework spec (`../Glitnir/docs/Platform/specs/2026-07-03-seeding-framework-design.md`): `ISeedContributor` lives inside the existing `.Migrations` project, no new assemblies.
